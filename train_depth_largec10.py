...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=45, shuffle=True, num_workers=2)
...
# net = ShuffleNetG2()
# net = SENet18()
net = depth_dynamic_slim.modulenet_c10()
net = net.to(device)
...
de = pd.DataFrame(density)
savepath='../outputs/'+str(args.mname)+'density01.csv'
de.to_csv(savepath)
...
for epoch in range(140+start_epoch, 140+start_epoch+20):
    best_graph = p1.graph_matrix
    print('\ntraining individual 1')
    backup(best_graph, best_acc,epoch,p1.connections, p='init')
    nd=train(epoch,p1.graph_matrix,p1.graph2,p1.graph3,p1.connections)
...
args.lr=0.0001
for epoch in range(160+start_epoch, 160+start_epoch+10):
    best_graph = p1.graph_matrix
    print('\ntraining individual 1')
    backup(best_graph, best_acc,epoch,p1.connections, p='init')
    nd=train(epoch,p1.graph_matrix,p1.graph2,p1.graph3,p1.connections)
