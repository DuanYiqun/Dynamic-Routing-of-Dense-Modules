...
class Modulenet(nn.Module):
    def __init__(self, block, nblocks, graph ,growth_rate=32, reduction=0.5, num_classes=100, core_nums=3):
        ...
        self.dw1 = Downsample1(52,98)
        self.dw2 = Downsample1(98,193)
        self.dw3 = Downsample1(52,193)
        
        """
        self.dw1 = Downsample1(40,68)
        self.dw2 = Downsample1(68,130)
        self.dw3 = Downsample1(40,130)
        """
        """
        self.dw1 = Downsample1(112,248)
        self.dw2 = Downsample1(248,508)
        self.dw3 = Downsample1(112,508)
        """
        """
        self.dw1 = Downsample1(142,323)
        self.dw2 = Downsample1(323,665)
        self.dw3 = Downsample1(142,665)
        """
    ...
    def forward(self,x, graph_matrix, graph1, graph2):
        out = self.conv1(x)
        #print(out.size())
        ...
        for ind, i in enumerate(graph_matrix[0]):
            for index,k in enumerate(i):
                if index == 0:
                    out_temp1.append(outs[k])
                    #print(out_temp[ind].size())
                    #print(k)
                else:
                    out_temp1[ind]=torch.add(out_temp1[ind],outs[k])
        outs=out_temp1
        ...
        for ind, i in enumerate(graph_matrix[1]):
            for index,k in enumerate(i):
                if index == 0:
                    out_temp2.append(outs[k])
                    #print(out_temp[ind].size())
                    #print(k)
                else:
                    out_temp2[ind]=torch.add(out_temp2[ind],outs[k])
            
        for ind, i in enumerate(graph1[0]):
            for index,k in enumerate(i):
                    out_temp2[ind]=torch.add(out_temp2[ind],self.dw1(out_temp1[ind]))
        outs=out_temp2
        ...
        for ind, i in enumerate(graph_matrix[1]):
            for index,k in enumerate(i):
                if index == 0:
                    out_temp3.append(outs[k])
                    #print(out_temp[ind].size())
                    #print(k)
                else:
                    out_temp3[ind]=torch.add(out_temp3[ind],outs[k])
        
        for ind, i in enumerate(graph1[1]):
            for index,k in enumerate(i):
                    out_temp3[ind]=torch.add(out_temp3[ind],self.dw2(out_temp2[ind]))

        for ind, i in enumerate(graph2[0]):
            for index,k in enumerate(i):
                    out_temp3[ind]=torch.add(out_temp3[ind],F.avg_pool2d(self.dw3(out_temp1[ind]), 2))
...
#print(net)
def modulenet_cifar():
    return Modulenet(Bottleneck,[6,12,24,16],graph_matrix, growth_rate=42,num_classes=100)

def modulenet_c10():
    return Modulenet(Bottleneck,[6,12,24,16],graph_matrix, growth_rate=42,num_classes=10)

def modulenet_c10_slim():
    return Modulenet(Bottleneck,[6,12,24,16],graph_matrix, growth_rate=12,num_classes=10)

def modulenet_c10_slim_8():
    return Modulenet(Bottleneck,[6,12,24,16],graph_matrix, growth_rate=8,num_classes=10)
...
def test_modulenet():
    net = modulenet_c10_slim()
    x = torch.randn(2,3,32,32)
    graph_matrix=[[[0],[1],[2]],[[0,2],[1,2],[2]],[[0,1],[1,2],[2]]]
    graph1 = [[[0],[1],[2]],[[0,2],[1,2],[2]]]
    graph2 = [[[0],[1],[2]]]
    y= net(Variable(x), graph_matrix, graph1, graph2 )
